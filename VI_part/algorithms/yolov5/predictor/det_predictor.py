...
class YOLOv5Detector(object):
    ...
    def from_config(cls, config):
        ...
        # ----------------------------------------
        return cls(infer_size, padding_color, conf_thres, iou_thres, device, net_conf, weight_path, classes, dnn, half)

    def __init__(self, infer_size, padding_color, conf_thres, iou_thres, device, net_conf, weight_path, classes, dnn, half):
        ...
        # ----------------------------------------
        # Load model
        self._model = DetectMultiBackend(weight_path, device=self._device , dnn=dnn, data=None, fp16=half)
        stride, names, pt = self._model.stride, self._model.names, self._model.pt
        self._names = names
        check_cls = [names[key] for key in names]
        if check_cls == classes:
            self._classes = classes
        else:
            print(f"check_cls != classes \n check_cls: {check_cls} \n classes: {classes}")
            self._classes = check_cls
    ...
    def _preprocess(self, image_list: list) -> tuple:
        ...
        for image in image_list:
            ...
            scala_ratio = min((self._infer_size[0]/image_shape[0]),
                              (self._infer_size[1]/image_shape[1]))
            ...
            # ------------------------------
            delta_height, delta_width = (self._infer_size[0]-image_scaled_shape[0],
                                         self._infer_size[1]-image_scaled_shape[1])
            # if auto
            ...
            image_tensor = image_tensor.half() if self._half else image_tensor.float()  
                                            # 精度减半 image_tensor.half() / image_tensor.float()
