...
class self_container_BaseRecLabelDecode(object):
    """ Convert between text-label and text-index """
...
class self_container_LabelDecode(self_container_BaseRecLabelDecode):
    """ Convert between text-label and text-index """

    def __init__(self, character_dict_path=None, use_space_char=False,
                 **kwargs):
        super(self_container_LabelDecode, self).__init__(character_dict_path,
                                             use_space_char)
...
class self_car_BaseRecLabelDecode(object):
    """ Convert between text-label and text-index """
    ...
    def __init__(self, character_dict_path=None, use_space_char=False):
        ...
        self.xk = ["36","24", "18" ,"22", "20", "21" ,"23" ,"16" ,"17" ,"26" ,"32", "25", "19", "31"]
        self.y = ["3", "4", "6", "7","8", "9", "10", "15","16", "17", "18", "19", "20", "21", "23", "25", "27", "28", "29", "33", "35", "36", "37", "39", "51"
    ...
    def decode(self, text_index, text_prob=None, is_remove_duplicate=False):
        ...
        for batch_idx in range(batch_size):
            ...
            if(len(new_text)>0):
                if(new_text[0]=='Y'):
                    if(len(new_text)>3):
                        if(new_text[1]+new_text[2] in self.y):
                            new_text = new_text[0]+new_text[1]+new_text[2]
                        elif(new_text[1]+new_text[3] in self.y):
                            new_text = new_text[0]+new_text[1]+new_text[3]
                        elif(new_text[2]+new_text[3] in self.y):
                            new_text = new_text[0]+new_text[2]+new_text[3]
                elif(new_text[0]=='X'):
                    if(len(new_text)>4):
                        if(new_text[1]+new_text[2] in self.xk):
                            new_text = new_text[0]+new_text[1]+new_text[2]
                        elif(new_text[1]+new_text[3] in self.xk):
                            new_text = new_text[0]+new_text[1]+new_text[3]
                        elif(new_text[2]+new_text[3] in self.xk):
                            new_text = new_text[0]+new_text[2]+new_text[3]
            text = new_text
            # ========================================
            text = text.replace(" ", "")
...
class self_car_CTCLabelDecode(self_car_BaseRecLabelDecode):
    """ Convert between text-label and text-index """

    def __init__(self, character_dict_path=None, use_space_char=False,
                 **kwargs):
        super(self_car_CTCLabelDecode, self).__init__(character_dict_path,
                                             use_space_char)

    def reset_first(self, arr, idx_0):
        positions = [41, 21, 42]
        for idx in range(0,len(arr[0][0])):
            if(idx not in positions):
                arr[0][idx_0][idx]=0
        return arr
    
    def reset_sec_alp(self,arr, idx_0, sec_idx):
        positions = [28, 37]
        for i in range(idx_0+1,sec_idx+1):
            # print(len(arr[0][0]))
            for idx in range(1,len(arr[0][0])):
                # if(i==7 and idx==27):
                    # print("heere")
                if(idx not in positions):
                    arr[0][i][idx]=0
    ...
    def reset_num(self, arr, sec_idx):
        positions = [x for x in range(1,11)]
        # print(arr.)
        # print("num", positions, len(arr))
        for i in range(sec_idx+1, len(arr[0])-1):
            for idx in range(1,len(arr[0][0])-1):
                if(idx not in positions):
                    arr[0][i][idx]=0
        return arr
    ...
    def __call__(self, preds, label=None, *args, **kwargs):
        ...
        for i in range(0, 10):
            if(preds_idx[0][i] == 0):
                ...
                idx_0 = i
                break
        preds = self.reset_first(preds,idx_0)
        preds_idx_test = preds.argmax(axis=2)[0]
        # print(preds.argmax(axis=2)[0])
        sec_idx = 1
        # print(preds_idx_test[0])
        if(preds_idx_test[idx_0] != 42):
            flag_sec = True
            myword = 0
            for idx in range(idx_0+1, idx_0+10):
                if(preds_idx_test[idx]==0):
                    if flag_sec:
                        continue
                    else:
                        sec_idx = idx - 1
                    ...
                    if(myword != 0 and preds_idx_test[idx]!= myword):
                        sec_idx = idx - 1
                        break
                    else:
                        sec_idx = idx
                        flag_sec = False
                        myword = preds_idx_test[idx]
            
            preds = self.reset_sec_alp(preds, idx_0, sec_idx)
        
            preds = self.reset_num(preds, sec_idx)
        else:
            preds = self.reset_num(preds, idx_0)
