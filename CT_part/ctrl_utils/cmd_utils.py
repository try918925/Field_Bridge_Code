...
def stop():
    ...
    send_msg["data"] = [
                        ...
                        {"cmd": "spreader_mid_lock", "value":int(20)}
                    ]
    set_global('CT001', send_msg)
    send_msg = json.dumps(send_msg, ensure_ascii=False)
    CT2MC_pub.send_msg(send_msg)
    ...
    send_msg["data"] = [
                        {"cmd": "trolley_velocity", "value":0.0},
                        {"cmd": "hoist_velocity", "value":0.0},
                        {"cmd": "spreader_mid_lock", "value":int(10)}
                    ]
    set_global('CT001', send_msg)
    send_msg = json.dumps(send_msg, ensure_ascii=False)
    CT2MC_pub.send_msg(send_msg)
...
def set_velocity(trolley_velocity: float, hoist_velocity: float):
    ...
    # send_msg = json.dumps(send_msg, ensure_ascii=False)
    # CT2MC_pub.send_msg(send_msg)
    set_cmd([{"cmd": "trolley_velocity", "value": trolley_velocity},{"cmd": "hoist_velocity", "value": hoist_velocity}])
    ...
    if mode == 'up': # 升导板
        ...
        while True: # 控制升导板 直到plc反馈执行完
            try:
                ...
                # send_msg = json.dumps(send_msg, ensure_ascii=False)
                # CT2MC_pub.send_msg(send_msg)
                set_cmd([{'cmd': 'flipper', 'value': 0}])

                time.sleep(0.2)[{'cmd': 'flipper', 'value': 0}]
                MC001 = get_global('MC001')
