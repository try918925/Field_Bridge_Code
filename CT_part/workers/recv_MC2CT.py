...
class thread_MC101(threading.Thread):
    def __init__(self, ):
        ...
        self.MC101_processing = copy.deepcopy(MC101)
    ...
    def run(self, ):      
        while True: # 每个循环为收取到任务
            ...
            if (work_type == "LOAD") and (actionType == 3):
                flip_thread = threading.Thread(target=set_flip, args=('up', [1, 2, 3, 4]))
                flip_thread.start()
            ...
            if (work_type == "DSCH") and (actionType == 3):
                try:
                    ...
                    print(error)
                    down_list = [1, 2, 3 ,4]
                flip_thread = threading.Thread(target=set_flip, args=('up', down_list))
                flip_thread.start()
            ...
            if actionType == 2: # 抓箱时需要判断吊具尺寸
                print('set_spreader')
                spreader_thread = threading.Thread(target = set_spreader, args=(self.MC101_processing["data"]["currentAction"]["workflow"], \
            ...
            if target_y > 60:
                if target_z > 15:
                    ...
                    self.open_loop_mv(target_y, target_z, self.MC101_processing)
                    # self.monitor()
                else:
                    print(target_y, 30, 20)
                    key_logger.info('anti_swing_mv to {}, {}, 15)'.format(target_y, max(z_start, 20)))
                    self.open_loop_mv(target_y, max(target_z, 20), self.MC101_processing)
                    # self.monitor()
            else:
                if target_z > 15:
                    print(target_y, max(z_start, 20, target_z), target_z)
            ...
            if (work_type == "LOAD") and (actionType == 2):
                ...
                flip_thread = threading.Thread(target=set_flip, args=('down', down_list))
                flip_thread.start()
            ...
            if actionType == 2 and self.MC101_processing["data"]["currentAction"]["work_type"] == "LOAD":
                ...
                while True:
                    ...
                    if MC001['data']['lock_state'] == 1:
                        ...
                        self.fast_hosit(0.3, MC001['data']['hoist_height']+0.3, False, self.MC101_processing)
                        # ！！！ todo check 是否安全
                        time.sleep(1.0)
                    ...
                    if sleep_cnt > 10:
                        ...
                        request_manual_mode(self.MC101_processing)
                        break      
            # 卸船放箱完成时, 有着箱信号开锁，无着箱信号时请求且手动，并且任务中断
    ...
    def fast_hosit(self, v: float, target_hoist, need_complete: bool, MC101_processing):
        ...
        while True:
            ...
            if 0.0 <= (z-target_hoist) < 0.4:
                exit_count += 1
            else:
                exit_count = 0
            
            if z > 15.5 and need_complete: # 起升到移动高度时给出任务结束
                key_logger.info('cost time: {}s'.format(time.time() - t0))
                set_velocity(0.0, 0.0) 
                time.sleep(0.04)
            ...
            if exit_count >= 1.0:
                ...
                key_logger.info('cost time: {}s'.format(time.time() - t0))
                return True # 顺利执行完
            set_velocity(0.0, float(v/10*count))
    ...
    def anti_swing_mv(self, target_y, target_z_1, target_z_2, MC101_processing):
        ...
        if y_start < target_y:
            target_y += 0.04
        elif y_start > target_y:
            target_y -= 0.04
        hoist_para = PIDParameters(kp= 0.5, 
                                ki= 0.0, 
                                kd= 0.0, 
                                lower= -2.0, 
                                upper= 2.0
        ...
        freeze_count = 0
        last_u = 0
        last_theta = get_global('MC114')['data']['spreader_target_theta']
        # main control loop
        while True:
            time.sleep(0.115)
            if check_new_task(MC101_processing) or self.stop_flag: # check new task id 
            ...
            if MC001['data']['wind'] > 15.0: # 风速大于10m时小车限速
            ...
            if freeze_count > 20:
                ...
                Ep = {
                        ...
                        "detail":f"{last_theta}|{theta}", #string,具体信息描述
                        "happen_time": int(time.time()*1000), #long, 时间戳,单位毫秒
                        "has_solved": False, #bool,是否已解决
                        "solve_time": None #long, 时间戳,单位毫秒
                    }
                ...
                send_msg = json.dumps(CT007, ensure_ascii=False)
                CT2MC_pub.send_msg(send_msg)
                change_CT002_state(MC101_processing ,"abort", False, False, reject_reason='theta feedback error') 
                error_logger.error(f'theta feedback error last_theta:{last_theta}, theta:{theta}')

            if abs(theta - last_theta) > 0.25:
                self.stop_flag =True
                stop()
                Ep = {
                        "exception_code": 'E102',#int,异常代码
            ...
            if abs(last_u - vy) > 3.0: # 失速后任务失败
                self.stop_flag = True
                stop()
                Ep = {
                        "exception_code": 'E128',#int,异常代码
                        "detail":f"{last_u}|{vy}", #string,具体信息描述
                        "happen_time": int(time.time()*1000), #long, 时间戳,单位毫秒
                        "has_solved": False, #bool,是否已解决
                        "solve_time": None #long, 时间戳,单位毫秒
                    }
                ...
                send_msg = json.dumps(CT007, ensure_ascii=False)
                CT2MC_pub.send_msg(send_msg)
                change_CT002_state(MC101_processing ,"abort", False, False, reject_reason='velocity response error') 
                error_logger.error(f'velocity response error u:{last_u}, v:{vy}')
                

            t = time.time() - t0 
            
            if abs(y - y_start) < min(10, abs(y_start-target_y)/2):
                target_z = target_z_1
            if abs(y - target_y) < min(10, abs(y_start-target_y)/2):
                target_z = target_z_2
            ...
            if abs(y-target_y) <= 0.05 and abs(theta)< 0.05 and (0.0 <= (z-target_z) < 0.05) and (vy < 0.5):
                exit_count += 1
            else:
                exit_count = 0
            if exit_count >= 1.0:
            ...
            t_list.append(t)
        # np.save('./log/controlled.npy', np.vstack([np.array(t_list), np.array(theta_list)]))
    ...
    def monitor(self):
        ...
        while t < 5:
            t = time.time() - t0
            theta = get_global('MC114')['data']['spreader_target_theta']
            theta_list.append(abs(theta))
            time.sleep(0.115)
        time_str = time.strftime('%Y-%m-%d %H:%M:%S', time.localtime())
        new_data = [time_str, max(theta_list)]
        with open('./log/theta_record.csv', mode='a', newline='', encoding='utf-8') as file:
            writer = csv.writer(file)
            writer.writerow(new_data)
        print("Swinging angle recorded.")

    def open_loop_mv(self, target_y, target_z, MC101_processing):
        MC001 = get_global('MC001')
        ...
        hoist_controller = PIDController(hoist_para)
        velocity_planner = FastPlanner(V=3.8, l=46-target_z, start=y_start, end=target_y)
        last_theta = get_global('MC114')['data']['spreader_target_theta']
        last_u = 0
        t0 = time.time()
        t_list = []
        ...
        while True:
            ...
            # t_list.append(t)
            # theta_list.append(theta)
            if MC001['data']['ctr_mod_auto'] == 0: # 切手动后任务失败
                stop() 
                change_CT002_state(MC101_processing ,"abort", False, False, reject_reason='auto change to manual') 
                break
            if judge_stop():
            ...
            if abs(theta - last_theta) > 0.25: # 吊具姿态更新堵塞时任务失败
                self.stop_flag =True
                stop()
                Ep = {
                        "exception_code": 'E102',#int,异常代码
            ...
            if abs(y-y_start) > abs(y-target_y) and vy == 0.0:
                print('Trolley Arrives.')
                key_logger.info('cost time: {}s'.format(time.time() - t0))
                stop()
                break 
            ...
            last_theta = theta
            last_u = uy
        # np.save('./log/un_controlled.npy', np.vstack([np.array(t_list), np.array(theta_list)]))
    ...
    def land_cntr(self, target_y, target_z, MC101_processing):
        ...
        hoist_controller = PIDController(hoist_para)
        last_lock_action = 0
        error_predictor = DLPredictor('./weights/weight_error.pth')
        zeta_predictor = DLPredictor('./weights/weight_angle.pth')
        theta_predictor = DLPredictor('./weights/weight_theta.pth')
        iou_limits = {
            "20f":{
                '1':0.45,
                '2':0.45,
        ...
        land_z = 4.75
        spreader_size = self.MC101_processing["data"]["currentAction"]["spreaderSize"]
        MC108 = get_global('MC108')
        lane_id = MC108['data']['lane_id']
        iou_limit = iou_limits[spreader_size][lane_id]
        lock_limit_up = lock_limits[lane_id]['up']
        lock_limit_low = lock_limits[lane_id]['low']
        error_x_limit_up = error_x_limits[lane_id]['up']
        error_x_limit_low = error_x_limits[lane_id]['low']
        latch_limit = lock_limits[lane_id]['latch']
        offset = offsets[lane_id]
        count = 0
        lock_count = 0
        flag = 0
        lift_flag = 0
        ...
        objectHeight = self.MC101_processing["data"]["currentAction"]["objectHeight"] if "objectHeight" in self.MC101_processing["data"]["currentAction"].keys() else None
        cntr_dh = 0.38 if (objectHeight == 2.9) else 0
        land_z = 5.3 if (objectHeight == 2.9) else 4.75
        # hover_z = target_z + dt+0.17
        hover_z = target_z + dt + offset + cntr_dh
        # main control loop
        while True:
            ...
            latch_dis = TEMP['lock_dis']['latch_dis']
            lock_ratio = lock_ratio_real
            if actionType == 3 or spreader_size != 'D20f' or lock_ratio == None: 
                lock_ratio = (lock_limit_up + lock_limit_low) / 2
                latch_dis=latch_limit - 1
            ...
            MC108 = get_global('MC108')
            if actionType == 3:
                target_zeta = MC108['data']['car_target_t']
            else:
                target_zeta = MC108['data']['cntr_target_t']                
            # cntr_zeta = MC108['data']['cntr_target_t']
        
            # error_x = MC114['data']['spreader_target_y']    
            # if error_x < 0:
            #     error_x += 0.14
            # elif error_x > 0:
            #     error_x -= 0.14
            ...
            try:
                ...
                error_x = cal_error_x(spreader_bbox, target_bbox)
            except:
                iou = 0
                error_x = 0
            if actionType == 3: 
                error_x = error_x_limit_up - 1
            error_pred = error_predictor.predict(error, if_smooth=False)
            zeta_pred = zeta_predictor.predict_max(zeta, offset=target_zeta)
            # theta_pred = theta_predictor.predict(theta-0.015, if_smooth=False)
            theta_max_pred = theta_predictor.predict_max(theta-0.015)
            # iou_pred = iou_predictor.predict(t, iou, dt)
            t_list.append(t)
            error_list.append(target_y - y - (sin(theta)*(46-rope_length)))
            zeta_list.append(zeta)
            iou_list.append(iou)
            theta_list.append(theta)
            cmd_list = []
            print(f'lock_ratio:{lock_ratio}, Amplitude:{theta_max_pred:.4f}, state:{state}, zeta:{zeta_pred:.3f}, error_x:{error_x}, latch_size:{latch_dis}, target_zeta:{target_zeta:.4f}')
            ...
            if state == 'lowering':
                ...
                count += 1
                actionType = self.MC101_processing["data"]["currentAction"]["actionType"] if "actionType" in self.MC101_processing["data"]["currentAction"].keys() else None
                error_pred_limit = 0.050 if actionType==2 else 0.042 # 抓放的条件阈值不同
                if (abs(theta_max_pred)<0.0045 and (lock_limit_low <= lock_ratio <= lock_limit_up or lock_count >= 1 or flag) \
                    and abs(zeta_pred)<0.012 and abs(error_x)< error_x_limit_up) and count > 10 or MC001['data']['land_state'] == 1:
                    # and abs(zeta_pred)<0.009 and abs(error_x)< error_x_limit_up) and count > 10 or MC001['data']['land_state'] == 1:
                    state = 'touching'
                    key_logger.info(f'hovering -> touching')
                    if last_lock_action == 21 or last_lock_action == 11:
                        cmd_list.append({"cmd": "spreader_mid_lock", "value": int(last_lock_action-1)})
                        set_cmd(cmd_list)
                    continue
                else:
                    if latch_dis < latch_limit: # 标准位附近，微张中锁
                        if lock_count < 12:
                            lock_action = 21
                            last_lock_action = lock_action
                            cmd_list.append({"cmd": "spreader_mid_lock", "value": int(lock_action)})
                            lock_count += 1
                        elif last_lock_action == 21:
                            lock_action = int(last_lock_action-1)
                            last_lock_action = lock_action
                            cmd_list.append({"cmd": "spreader_mid_lock", "value": int(lock_action)})
                    else: # 非标准位，按照比例调节
                        if lock_limit_low <= lock_ratio <= lock_limit_up:
                            flag = 1
                        else:
                            flag = 0
                        if lock_ratio > lock_limit_up and flag == 0:
                            lock_action = 21
                            last_lock_action = lock_action
                            cmd_list.append({"cmd": "spreader_mid_lock", "value": int(lock_action)})
                        elif last_lock_action == 21:
                            ...
                            last_lock_action = lock_action
                            cmd_list.append({"cmd": "spreader_mid_lock", "value": int(lock_action)})
                        if lock_ratio < lock_limit_low and flag == 0:
                            lock_action = 11
                            last_lock_action = lock_action
                            cmd_list.append({"cmd": "spreader_mid_lock", "value": int(lock_action)}) 
                        elif last_lock_action == 11:
                            lock_action = 10
                            last_lock_action = lock_action
                            cmd_list.append({"cmd": "spreader_mid_lock", "value": int(lock_action)})            
                ...
                if MC001['data']['land_state'] == 1:
                    ...
                    time.sleep(5.0)
                    break
                elif z <= land_z and lift_flag == 0 and actionType == 2 and (spreader_size == 'D20f' or spreader_size == '40f'):
                # elif z <= 4.7 and lift_flag == 0 and actionType == 2 and spreader_size == 'D20f':
                    cmd_list.append({"cmd": "hoist_velocity", "value": 0.0})
                    set_cmd(cmd_list)
                    state = 'lifting'
                    key_logger.info(f'touching -> lifting')
                    lift_flag = 1
                else:
                    uz = hoist_controller.calculate_output(target_z, z)
                    if abs(0.4*uz) < 0.4:
                ...
                if z <= land_z + 0.7: 
                    uz = 0.1
                    cmd_list.append({"cmd": "hoist_velocity", "value": uz})
                    set_cmd(cmd_list)
                else:
                    count = 0
                    # lock_count = 0
                    ...
                    stop()
                    state = 'hovering'
                    key_logger.info(f'lifting -> hovering')
                

        stop()
        formatted_time = str(self.MC101_processing['timestamp'])
        np.save(f'./log/data/t-{formatted_time}.npy', np.array(t_list))
        np.save(f'./log/data/error-{formatted_time}.npy', np.array(error_list))
        np.save(f'./log/data/iou-{formatted_time}.npy', np.array(iou_list))
        np.save(f'./log/data/zeta-{formatted_time}.npy', np.array(zeta_list))
        np.save(f'./log/data/theta-{formatted_time}.npy', np.array(theta_list))
...
#    try:
        ...
        print('task', run_task_id, new_task_id)
...
def process_MC106():
    '''
        贝扫任务
    '''
    # {"craneId": "404-1", "data": {"scan_cmd_index": 2, "trolley_start_pos": 55.0, "trolley_target_pos": 85.0}, "msg_name": "MC106", "msg_uid": "61934d22-293d-4455-8153-f6a77d553036", "receiver": "CT", "sender": "MC", "timestamp": 1721896549351} 
    def anti_swing_mv(target_y, target_z_1, target_z_2):
        MC001 = get_global('MC001')
        y_start, z_start = MC001['data']['trolley_pos'], MC001['data']['hoist_height']
        hoist_para = PIDParameters(kp= 0.5, 
                                ki= 0.0, 
        ...
        trolley_controller = CraneController(3.2, 0.6, pos_para, angle_para)
        trolley_controller.set(y_start, target_y)
        hoist_controller = PIDController(hoist_para)
        t0 = time.time()
        y_list, vy_list, yd_list, uy_list = [], [], [], []
        ...
        while True:
            ...
            if t > 100:
                ...
                stop()
                break
            if abs(y-target_y) <= 0.3 and abs(theta)< 0.15 and (0.0 <= (z-target_z) < 0.2):
                exit_count += 1
            else:
                exit_count = 0
            if exit_count >= 1.0:
    ...
    CT005 = {
        ...
        "data": {
            ...
            "scan_state":"received",#string,任务状态：received,running,reject,abort,complete
        }
    }
    send_msg = json.dumps(CT005, ensure_ascii=False)
    CT2MC_pub.send_msg(send_msg)
    CT005 = {
        "msg_uid":str(uuid.uuid1()),#string,消息序号
        "msg_name":"CT005",#string,消息名称
    ...
    CT2MC_pub.send_msg(send_msg)
    anti_swing_mv(85.0, 35, 35)
    anti_swing_mv(55.0, 35, 35)
    ...
    CT005 = {
        ...
        "data": {
            "scan_cmd_index":1,#int,贝扫指令序号
            "scan_state":"complete",#string,任务状态：received,running,reject,abort,complete
        }
    }
    send_msg = json.dumps(CT005, ensure_ascii=False)
    CT2MC_pub.send_msg(send_msg)
        ...
        while True:
            ...
            if MC2CT_msg['msg_name'] in process_dict.keys():
                ...
                if MC2CT_msg['msg_name'] == 'MC114':
                    main_logger.debug(f'MC to CT is procesing - {json.dumps(MC2CT_msg)}')
                else:
                    main_logger.info(f'MC to CT is procesing - {json.dumps(MC2CT_msg)}')
