...
class thread_MC101(threading.Thread):
    ...
    def land_cntr(self, target_y, target_z, MC101_processing):
        ...
        while True:
            ...
            error_pred = predictor.predict(t, error, dt)
            t_list.append(t)
            error_list_1.append(error)
            error_list_2.append(error_pred)
            cmd_list = []
            print(f'error:{error:.3f}, pred_error:{error_pred:.3f}, state:{state}, land_state:{land_state}')
            ...
            if state == 'lowering':
                ...
                if abs(error_pred) < error_pred_limit or MC001['data']['land_state'] == 1:
                    state = 'touching'
